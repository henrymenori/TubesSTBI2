...
Collection.prototype.balancing = function(query, retrieval, relopt, tops, sameopt, expopt) {
	...
	var countrel;
	var countirr;
	if(expopt == 'option2'){ // no query expansion
		if(relopt == 'option1' || relopt == 'option4') {
			for(var j = 0; j < wordlength; j++) {
				...
				if(this.invertedfile[word[j]] != undefined) {
					...
					if(countrel == 0) countrel++;
					if(countirr == 0) countirr++;
	...
	else{ // using query expansion
		if(relopt == 'option1' || relopt == 'option4') {
			for(var key in this.invertedfile) {
				// console.log(key);
				sumrelevant = 0;
				sumirrelevant = 0;
				countrel = 0;
				...
				for(var k = 0; k < retrieval.length; k++) {
					if(this.invertedfile[key][retrieval[k].nodoc] != undefined) {
						if(retrieval[k].relevant) {
							sumrelevant = sumrelevant + this.invertedfile[key][retrieval[k].nodoc];
							countrel++;
						}
						else {
							sumirrelevant = sumirrelevant + this.invertedfile[key][retrieval[k].nodoc];
							countirr++;
						}
					}
				}
				if(countrel == 0) countrel++;
				if(countirr == 0) countirr++;
				if (invertedfile[key] == undefined) invertedfile[key] =  0;
				invertedfile[key] = invertedfile[key] + (sumrelevant / countrel) - (sumirrelevant / countirr);
				if(invertedfile[key] < 0) {
					invertedfile[key] = 0;
				}
			}
		}
			...
			for(var key in this.invertedfile) {
				...
				if (invertedfile[key] == undefined) invertedfile[key] =  0;
				invertedfile[key] = invertedfile[key] + sumrelevant - sumirrelevant;
				if(invertedfile[key] < 0) {
					invertedfile[key] = 0;
				}
			}
		...
		var index = 0;
		for(var key in invertedfile){
			if(invertedfile[key] == 0)	delete invertedfile[key];
		}
	}
	// console.log(invertedfile);
	...
	for(var i = 0; i < this.rawdocuments.length; i++) {
		similarity.push(0);
		for(var key in invertedfile) {//j = 0; j < wordlength; j++) {
			if(this.invertedfile[key] != undefined) {
				if(this.invertedfile[key][i + 1] != undefined) {
					similarity[i] = similarity[i] + invertedfile[key] * this.invertedfile[key][i + 1];
				}
			}
		}
		similarity[i] = similarity[i] / norm / this.norm[i];
	...
	// if not use same document
	if(sameopt == 'option2'){
		for(var i = 0; i < retrieval.length; i++){
			similarity[retrieval[i].nodoc-1] = 0;
		}
	}
...
Collection.prototype.experimental = function(querylocation, relevancelocation, relopt, tops, topn, sameopt, expopt) {
	...
	for(var i = 0; i < queries.length; i++) {
		...
		if(relopt != 'option4') {
			for(var j = 0; j < temp.result.length; j++) {
				if(relevances[i].indexOf(temp.result[j].nodoc) != -1) {
					temp.result[j]['relevant'] = true;
				}
				else {
					temp.result[j]['relevant'] = false;
				}
			}
		}
		else {
		...
		// console.log(temp);
		temp = this.balancing(queries[i], temp.result, relopt, tops, sameopt, expopt);
		...
		if(temp.result.length != 0) {
			precision = rank.length / temp.result.length;
		}
		else {
			precision = 0;
		}
...
Collection.prototype.interactive = function(query, tops) {
	var temp = this.indexing(query, tops);
	var result = [];
	
	for(var i = 0; i < temp.result.length; i++) {
		result.push({
			nodoc : temp.result[i].nodoc,
			title : this.rawdocuments[temp.result[i].nodoc - 1].title,
			words : this.rawdocuments[temp.result[i].nodoc - 1].words
		});
	}
...
Collection.prototype.interactive2 = function(query, relevance, relopt, tops, topn, sameopt, expopt) {
	var temp;
	var result = [];
	...
	temp = this.indexing(query, tops);
	
	if(relopt != 'option4') {
		for(var j = 0; j < temp.result.length; j++) {
			if(relevance.indexOf(temp.result[j].nodoc) != -1) {
				temp.result[j]['relevant'] = true;
			}
			else {
				temp.result[j]['relevant'] = false;
	...
	var temp2 = this.balancing(query, temp.result, relopt, tops, sameopt, expopt);
	console.log(temp2);
